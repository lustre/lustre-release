                    ...
                    unsigned int offset, int nob)
{
        ...
        if (tx == NULL) {
                CERROR ("Can't do %s rdma to "FMT_NID": tx descs exhausted\n",
        ...
        if (ptlrc != PTL_OK) {
                CERROR("PtlMDBind(%s) failed: %d\n",
                       libcfs_nid2str(peer->peer_nid), ptlrc);
                tx->tx_status = -EIO;
...
kptllnd_send(lnet_ni_t *ni, void *private, lnet_msg_t *lntmsg)
{
        ...
        case LNET_MSG_PUT:
                ...
                if (tx == NULL) {
                        CERROR("Can't send %s to %s: tx descs exhausted\n",
        ...
        case LNET_MSG_GET:
                ...
                if (tx == NULL) {
                        CERROR("Can't send GET to %s: tx descs exhausted\n",
                               libcfs_id2str(target));
                        return -ENOMEM;
                }
                   ...
                   lnet_msg_t *msg, void **new_privatep)
{
        ...
        /* I have to release my ref on rxb to ensure I'm an eager receiver, so
         * I copy the incoming request from the buffer it landed in, into space
         * reserved in the descriptor... */
        ...
        case PTLLND_MSG_TYPE_IMMEDIATE:
                ...
                if (nob > rx->rx_nob) {
                        CERROR ("Immediate message from %s too big: %d(%d)\n",
                                libcfs_nid2str(rx->rx_peer->peer_nid), nob,
                                rx->rx_nob);
                        rc = -EINVAL;
                        break;
                }
