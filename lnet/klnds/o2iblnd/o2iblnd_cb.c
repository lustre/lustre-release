...
again:
	peers_version = kiblnd_data.kib_peers_version;
        list_for_each (ptmp, peers) {
                peer = list_entry (ptmp, kib_peer_t, ibp_list);
again2:
		conns_version = peer->ibp_conns_version;
			...
			if (!kiblnd_conn_timed_out(conn)) {
				kiblnd_conn_decref(conn); /* ...until here */
				read_lock_irqsave(glock, flags);
				if (peers_version != kiblnd_data.kib_peers_version)
					goto again;
				/* we know the peer is still on peer table */
				if (conns_version != peer->ibp_conns_version)
					goto again2;
			...
			read_lock_irqsave(glock, flags);
			/* start again now if something changed */
			if (peers_version != kiblnd_data.kib_peers_version)
				goto again;
			else
				goto again2;
