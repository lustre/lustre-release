...
/* Since both LNet and libcfs are handling cpu_pattern this mean
 * if both the LNet and libcfs modules are loaded and you unload
 * then reload lnet.ko you can end up accessing a freeded memory
 * locations. To avoid this prevent the LNet module from freeing
 * this shared string between modules.
 */
static void cpu_pattern_free(void *arg)
{
}

static const struct kernel_param_ops param_ops_cpu_pattern = {
	.set	= param_set_charp,
	.get	= param_get_charp,
	.free	= cpu_pattern_free,
};
 ...
 */
#define param_check_cpu_pattern(name, p) \
	__param_check(name, p, char *)
module_param_cb(cpu_pattern, &param_ops_cpu_pattern, &cpu_pattern, 0444);
