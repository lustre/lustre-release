...
${LMC} --add mds --nspath /mnt/mds_ns --node localhost --mds mds1 --fstype $FSTYPE --dev $MDSDEV --mountfsoptions $MDS_MOUNT_OPTS --size $MDSSIZE $JARG $IARG || exit 20
...
${LMC} --add ost --nspath /mnt/ost_ns --node localhost --lov lov1 --fstype $FSTYPE --dev $OSTDEV --mountfsoptions $OST_MOUNT_OPTS --size $OSTSIZE $JARG || exit 30
